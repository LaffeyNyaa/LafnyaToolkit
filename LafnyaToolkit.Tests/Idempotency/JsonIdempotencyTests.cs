...
namespace LafnyaToolkit.Tests.Idempotency
{
    ...
    public sealed class JsonIdempotencyTests
    {
        ...
        public void TestIdempotency(bool unused)
        {
            ...
            RunCase("unicode string",
                "{\"name\":\"\\u4e2d\\u6587\",\"emoji\":\"\\ud83d\\ude00\"}");

            RunCase("escaped chars",
                "{\"q\":\"a\\\"b\\\\c\\/d\\bf\\ne\\rf\\tg\\u0001h\"}");
            RunCase("empty object", "{}");
            RunCase("empty array", "[]");

            RunCase("number formats",
                "{\"ints\":[0,-1,1,42,-999],\"floats\":[0.0,-0.0,1.5,1e10,2.5E-3],\"exponents\":[1e0,1E+10,1.0e-10]}");
            RunCase("long string", "{\"s\":\"" + new string('a', 100) + "\"}");

            RunCase("deeply nested",
                "{\"a\":{\"b\":{\"c\":{\"d\":{\"e\":{\"f\":1}}}}}}");
            ...
            RunCase("mixed types",
                "[null,true,false,0,\"\",{},[],{\"k\":\"v\"}]");
            RunCase("large object", BuildLargeObject(20));

            RunCase("null value variants",
                "{\"a\":null,\"b\":{\"c\":null},\"d\":[null,null]}");

            RunCase("strings with control chars",
                "{\"a\":\"line1\\nline2\\ttab\"}");
